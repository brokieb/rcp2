            ...
            _.initials = {
                ...
                dates: {
                    pl: {
                        ...
                        noEventForToday: "Ten dzień będzie dostępny dopiero jutro",
                        noEventForThisDay: "Nie zarejestrowałem pracy tego dnia",
                        previousYearText: "Poprzedni rok",
                        nextYearText: "Następny rok",
                        closeSidebarText: "Zamknij 1",
                        closeEventListText: "Zamknij 2"
                    }
                }
            }
            _.initials.weekends = {
...
//        }
    ...
    EvoCalendar.prototype.addEventList = function(event_data) {
        ...
        markup = '<div class="event-container" role="button" data-event-type="'+event_data.type+'" data-event-index="'+(event_data.id)+'">';
        markup += '<i>'+event_data.id+'</i>';
        ...
        switch(event_data.type){
            case '0':
                markup += '<div class="event-container custom-container">';
                markup += '<form method="POST">';
                markup += '<i>'+event_data.id+'</i>';
                markup += '<p class="event-title">Dodaj komentarz</p>';
                markup += '<input type="hidden" name="mode" value="8">';
                markup += '<input type="hidden" name="date" value="'+event_data.date+'">';
                markup += '<span>'+event_data.comment+'</span>';
                markup += '<input type="text" name="komentarz" autocomplete="off">';
                markup += '<button>Wyślij</button>';
                markup += '</form>';
                markup += '</div>';
                break;
            case '1':
                ...
                markup += '<p class="event-title">Dodaj komentarz</p>';
                markup += '<input type="hidden" name="mode" value="8">';
                markup += '<input type="hidden" name="id" value="'+event_data.id+'">';
                markup += event_data.comment;
                markup += '<input type="text" name="komentarz" autocomplete="off" >';
                markup += '<button>Wyślij</button>';
                markup += '</form>';
                markup += '</div>';
            ...
            case '2':
                ...
                break;
            case '3':
                markup += '<div class="event-container">';
                markup += event_data.comment;
                markup += '</div>';
                break;
        }
    ...
    EvoCalendar.prototype.buildSidebarMonths = function () {
        ...
        console.log("asd" + _.$active.month);
$(".count-hours strong").html(_.$active.month+1);
var ans = [];
var rem = $("#evoCalendar").attr("data-ids");

if(rem != "0"){
    $("#evoCalendar").evoCalendar("removeCalendarEvent",JSON.parse(rem));
}
$.ajax({
    type: "GET",
    url: "ajax/count-hours-calendar.php",
    data: {
      month: _.$active.month+1,
    },
    dataType: "json",
    success: function (zmienna) {
        console.log(zmienna);
     zmienna['ids'].forEach(function(x){
        ans.push(x['id']);
     });
     $("#evoCalendar").attr("data-ids",JSON.stringify(ans));
     $(".count-hours strong").html(zmienna['czas']);
     $("#evoCalendar").evoCalendar('addCalendarEvent',zmienna['ids']);
    }
  });
    ...
    EvoCalendar.prototype.addEventIndicator = function (event) {
        ...
        function appendDot(date) {
            ...
            if (thisDate.find('span.event-indicator > .type-bullet > .type-bul ' + type).length === 0) {
                htmlToAppend = '<div class="type-bullet"><div ';

                htmlToAppend += 'class="type-bul ' + event.type + '"'
                if (event.color) {
